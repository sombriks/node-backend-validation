...
export const prepareAddressesRequests = async options => {
	...
	return {
		...
		async update(context) {
			...
			if (!affected) log.warn('No address updated');
		...
		async del(context) {
			...
			if (!affected) log.warn('No address removed');
			context.status = 303;
		...
		people: {
			...
			async add(context) {
				...
				if (!affected) log.warn('No people added into address #' + addresses_id);
				context.status = 303;
			...
			async del(context) {
				...
				if (!affected) log.warn('No people removed from address #' + addresses_id);
				context.status = 303;
				context.set('Location', `/addresses/${addresses_id}/people`);
